...
import type { MessagesFail, MessagesOk } from '@/util/msg';

export type ResponseBase =
  | Response<any, Record<string, any>>
  | Promise<Response<any, Record<string, any>>>;

export type ResponseAdapterFunc = (
  res: Response,
  ...args: any[]
) => ResponseBase;
...
export type ResponseOk = {
  code: 0;
  type: MessagesOk | (string & {});
  [k: string]: any;
};

export type ResponseFail = {
  code: 1;
  type: MessagesFail | (string & {});
  [k: string]: any;
};

export type ResponseAdapter<
  T extends ResponseAdapterExtend = ResponseAdapterExtend,
> = {
...
export function extendResponseAdapter<T extends ResponseAdapterExtend>(
  patch:
    | Partial<ResponseAdapter<T>>
    | ((prev: ResponseAdapter) => ResponseAdapter<T>),
) {
