...
function init() {
  ...
  document.getElementById('umd').addEventListener('click', onClick);
  // onClick({ target: { id: 'esm' } });
}
const onClick = e => {
  console.clear();
  const type = e.target.id;

  window[`${type}Echo`]?.(async (echo, sample, HEX_COLORS, chalk) => {
    echo.css(
      {
    ...
    const save = chalk.fg('#1ff', 'black');
    console.log('chalk', save.bg('red', 'white')('Hello', {}, ['xxx'], 'World!'));
    console.log('chalk', save.bg('black', 'gray')('World', {}, ['yyy'], 'Hello!'));
  });
};
