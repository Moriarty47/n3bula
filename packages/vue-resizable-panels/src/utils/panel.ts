...
const getElementByAttributeName = (
...
export const getPanel = (id: string, scope: ParentNode | HTMLElement = document): HTMLElement | null =>
  getElementByAttributeName('data-panel-id', id, scope);

export const getPanelDataIndex = (panels: PanelData[], panelData: PanelData) =>
  panels.findIndex(panel => panel.id === panelData.id);

export const getPanelForGroup = (groudId: string, scope: ParentNode | HTMLElement = document): HTMLElement[] =>
  Array.from(scope.querySelectorAll(`[data-panel][data-panel-group-id="${groudId}"]`));
...
export const getResizeHandle = (id: string, scope: ParentNode | HTMLElement = document): HTMLElement | null =>
  getElementByAttributeName('data-resize-handle-id', id, scope);

export const getResizeHandleIndex = (
  groupId: string,
  id: string,
  scope: ParentNode | HTMLElement = document,
): number => {
...
export const getResizeHandlesForGroup = (
  groupId: string,
  scope: ParentNode | HTMLElement = document,
): HTMLElement[] => {
  return Array.from(scope.querySelectorAll(`[data-resize-handle-id][data-panel-group-id='${groupId}']`));
};

export const getResizeHandlePanelIds = (
  groupId: string,
  handleId: string,
...
export const panelsMapToSortedArray = (panels: Panels): PanelData[] =>
  Array.from(panels.values()).sort((a, b) => a.order! - b.order!);
...
export const panelDataHelper = (
  ...
  const pivotIndices: PivotIndices = isLastPanel ? [panelIndex - 1, panelIndex] : [panelIndex, panelIndex + 1];
