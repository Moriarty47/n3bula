...
import { KeyValueCell } from '@/cell';
import { PAGE_SIZE } from '@/constants';
import { BTreeNode, findIndexOfFirstGreaterElement } from '@/btreenode';
...
test('binaryFindFirstGreatorElement', () => {
  ...
  i = findIndexOfFirstGreaterElement(arr, 3, comparator);
  expect(i).toBe(3);
  expect(arr[i]).toBe(4);

  i = findIndexOfFirstGreaterElement(arr, 10, comparator);
  expect(i).toBe(-1);
  expect(arr[i]).toBeUndefined();

  i = findIndexOfFirstGreaterElement(arr, 0, comparator);
  expect(i).toBe(0);
  expect(arr[i]).toBe(1);
  ...
  i = findIndexOfFirstGreaterElement(arr, 1, comparator);
  expect(i).toBe(-1);

  arr = [1, 1, 1, 1, 1, 1, 1, 1, 1];
  i = findIndexOfFirstGreaterElement(arr, 2, comparator);
  expect(i).toBe(-1);
